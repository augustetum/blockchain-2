...
bool MerkleTree::verify(const std::string& data, const std::vector<std::string>& proof, 
    ...
    for (const auto& proofHash : proof) {
        ...
        // Use the one that would lead to root (in practice, we'd need to know ordering)
        // For simplicity, we try left position first
        currentHash = hash1;
    }
...
void MerkleTree::printNode(const std::shared_ptr<Node>& node, int depth) const {
    ...
    std::cout << indent << node->hash.substr(0, 8) << "..." << std::endl;
    
    if (node->left) printNode(node->left, depth + 1);
    if (node->right && node->right != node->left) printNode(node->right, depth + 1);
}
